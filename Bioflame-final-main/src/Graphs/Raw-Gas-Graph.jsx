...
// CO2 removed — only tracking CH4 in raw gas
...
export default function RawGasGraph({ filterPeriod, selectedDate }) {
  ...
  useEffect(() => {
    async function fetchGraphData() {
      ...
      if (ch4Error) {
        console.error("Supabase Error fetching Raw Gas data:", ch4Error);
      ...
      {!loading && (
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={graphData} margin={{ top: 20, right: 40, left: 20, bottom: 60 }}>
            ...
            <Line
              type="linear" dataKey="ch4" stroke="#6C8E3E" strokeWidth={3}
              dot={{ r: 4 }} activeDot={{ r: 6 }}
              name="Methane (CH₄)" isAnimationActive={false} connectNulls={true}
            />
          </LineChart>
        </ResponsiveContainer>
      )}
